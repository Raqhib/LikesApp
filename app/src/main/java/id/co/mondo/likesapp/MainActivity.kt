...
import id.co.mondo.likesapp.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun showMouth(isHappy: Boolean){
        when(isHappy){
            true -> {
                ...
                mPaint.color = ResourcesCompat.getColor(resources, R.color.white, null)
                val mouth = RectF(halfOfWidth - 180F, halfOfHeight, halfOfWidth + 180F, halfOfHeight + 300F)
                mCanvas.drawArc(lip, 25F, 130F, false, mPaint)
            }
            false -> {
                mPaint.color = ResourcesCompat.getColor(resources, R.color.black, null)
    ...
    private val messsage = "Apakah Kamu suka bermain?"
    ...
    private fun showText(){
        ...
        val mBounds = Rect()
        mPaintText.getTextBounds(messsage, 0, messsage.length, mBounds)

        val x: Float = halfOfWidth - mBounds.centerX()
        val y = 50F
        mCanvas.drawText(messsage, x, y, mPaintText)
    }
